class ProjectsController < ApplicationController
  ...
  def add_user
    ...
    respond_to do |format|
      if @project_user.save
        ...
        format.html{redirect_to users_tenant_project_url(id: @project.id, tenant_id: @project.teant_id),
  ...
  def project_params
      params.require(:project).permit(:title, :detalis, :expected_completion_date, :tenant_id)
  end
  ...
  def verify_tenant
    unless params[:tenant_id] == Tenant.current_tenant_id.to_s
        redirect_to :root,
        flash: {error: 'You are not authorized to access any organization other then your own'}
    end
  end 
end
